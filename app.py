...
if "GEMINI_API_KEY" in st.secrets:
    ...
    # 如果你在本地測試，且還沒設定 secrets，可以先手動填入測試
    api_key = st.sidebar.text_input("請輸入 Gemini API Key", type="password")

# 3. 核心抓取與分析功能
def fetch_and_analyze():
    if not api_key:
        st.warning("⚠️ 請先設定或輸入 Gemini API Key！")
        return None
    ...
    with st.spinner("🔄 正在擷取國藝會最新網頁文字..."):
        try:
            ...
            # 為了省 Token，我們只抓網頁的純文字，把不必要的 HTML 標籤先濾掉
    ...
    with st.spinner("🧠 Gemini API 正在進行語意分析與結構化萃取..."):
        try:
            ...
            【嚴格規範】
            1. 必須以 JSON Array 格式輸出。
            2. 每個物件必須包含四個欄位：
               - "date": 職缺公告或更新日期 (格式如: 2026-07-09)
               - "organization": 求才單位名稱
               - "title": 職缺職稱
               - "link": 該職缺的詳細網址 (如果從文字中無法精確取得，請填寫: https://www.ncafroc.org.tw/recruitment.html)
