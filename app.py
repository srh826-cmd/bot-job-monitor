...
st.caption("使用 Streamlit + Google AI Studio (Gemini 2.5 Flash)")
...
def fetch_and_analyze():
    ...
    with st.spinner("🧠 Gemini API 正在過濾並排版今日最新職缺..."):
        try:
            ...
            # 💡 補回優化：啟用 Gemini 2.5 專屬的結構化 JSON Mode，速度最快、且 100% 不會發生解析崩潰
            ...
            2. 每個物件必須包含四個欄位：
               ...
               - "organization": 求才單位名稱
               - "title": 職缺職稱
               - "link": 請精準填寫該職缺對應的詳細內頁網址。如果真的找不到獨立內頁網址，才允許填寫: https://www.ncafroc.org.tw/recruitment.html。
            ...
            # 啟用 response_mime_type 後，response.text 保證是一條絕對乾淨的 JSON 字串
